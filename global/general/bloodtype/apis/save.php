...
/**
 * global/general/bloodtype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header bloodtype (mst_bloodtype)
 *
 ...
 * tanggal 09/06/2024
 */
$API = new class extends bloodtypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_bloodtype';
		$primarykey = 'bloodtype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\bloodtype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new bloodtype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"bloodtype_id" => $obj->bloodtype_id
				];

				$criteriaValues = [
					"bloodtype_id" => " bloodtype_id = :bloodtype_id "
				];
				...
				$sqlFieldList = [
					'bloodtype_id' => 'A.`bloodtype_id`', 'bloodtype_name' => 'A.`bloodtype_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_bloodtype A";
				$sqlWhere = $where->sql;
