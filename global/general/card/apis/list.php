...
/**
 * global/general/card/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header card (mst_card)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends cardBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\card_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new card_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.card_id LIKE CONCAT('%', :search, '%') OR A.card_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'card_id' => 'A.`card_id`', 'card_name' => 'A.`card_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_card A";
			$sqlWhere = $where->sql;
