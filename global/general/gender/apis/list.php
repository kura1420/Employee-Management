...
/**
 * global/general/gender/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header gender (mst_gender)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends genderBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\gender_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new gender_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.gender_id LIKE CONCAT('%', :search, '%') OR A.gender_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'gender_id' => 'A.`gender_id`', 'gender_name' => 'A.`gender_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_gender A";
			$sqlWhere = $where->sql;
