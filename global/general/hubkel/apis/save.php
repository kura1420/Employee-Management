...
/**
 * global/general/hubkel/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header hubkel (mst_hubkel)
 *
 ...
 * tanggal 10/06/2024
 */
$API = new class extends hubkelBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_hubkel';
		$primarykey = 'hubkel_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\hubkel_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new hubkel_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"hubkel_id" => $obj->hubkel_id
				];

				$criteriaValues = [
					"hubkel_id" => " hubkel_id = :hubkel_id "
				];
				...
				$sqlFieldList = [
					'hubkel_id' => 'A.`hubkel_id`', 'hubkel_name' => 'A.`hubkel_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_hubkel A";
				$sqlWhere = $where->sql;
