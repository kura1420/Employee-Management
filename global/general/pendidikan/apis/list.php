...
/**
 * global/general/pendidikan/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header pendidikan (mst_pnddk)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends pendidikanBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\pendidikan_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new pendidikan_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.pnddk_id LIKE CONCAT('%', :search, '%') OR A.pnddk_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'pnddk_id' => 'A.`pnddk_id`', 'pnddk_name' => 'A.`pnddk_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_pnddk A";
			$sqlWhere = $where->sql;
