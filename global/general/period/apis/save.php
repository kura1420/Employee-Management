...
/**
 * global/general/period/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header period (mst_period)
 *
 ...
 * tanggal 09/06/2024
 */
$API = new class extends periodBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_period';
		$primarykey = 'period_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\period_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new period_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->period_id = strtoupper($obj->period_id);


			if ($obj->period_descr=='') { $obj->period_descr = '--NULL--'; }
			...
			try {
				...
				$sqlFieldList = [
					'period_id' => 'A.`period_id`', 'period_name' => 'A.`period_name`', 'period_descr' => 'A.`period_descr`', 'period_isdisabled' => 'A.`period_isdisabled`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_period A";
				$sqlWhere = $where->sql;
