...
/**
 * global/general/religion/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header religion (mst_religion)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends religionBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\religion_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new religion_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.religion_id LIKE CONCAT('%', :search, '%') OR A.religion_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'religion_id' => 'A.`religion_id`', 'religion_name' => 'A.`religion_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_religion A";
			$sqlWhere = $where->sql;
