...
/**
 * global/general/worktype/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header worktype (mst_worktype)
 *
 ...
 * tanggal 09/06/2024
 */
$API = new class extends worktypeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_worktype';
		$primarykey = 'worktype_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\worktype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new worktype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"worktype_id" => $obj->worktype_id
				];

				$criteriaValues = [
					"worktype_id" => " worktype_id = :worktype_id "
				];
				...
				$sqlFieldList = [
					'worktype_id' => 'A.`worktype_id`', 'worktype_name' => 'A.`worktype_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_worktype A";
				$sqlWhere = $where->sql;
