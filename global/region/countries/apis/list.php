...
/**
 * global/region/countries/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header countries (countries)
 ...
 * tanggal 10/06/2024
 */
$API = new class extends countriesBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\countries_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new countries_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'id' => 'A.`id`', 'name' => 'A.`name`', 'iso2' => 'A.`iso2`', 'iso3' => 'A.`iso3`',
				'phone_code' => 'A.`phone_code`', 'postcode_required' => 'A.`postcode_required`', 'is_eu' => 'A.`is_eu`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "countries A";
			$sqlWhere = $where->sql;
