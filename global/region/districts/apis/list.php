...
/**
 * global/region/districts/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header districts (districts)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends districtsBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\districts_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new districts_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'id' => 'A.`id`', 'name' => 'A.`name`', 'regency_id' => 'A.`regency_id`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "districts A";
			$sqlWhere = $where->sql;
