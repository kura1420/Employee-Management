...
/**
 * global/region/provinces/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header provinces (provinces)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends provincesBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\provinces_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new provinces_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'id' => 'A.`id`', 'name' => 'A.`name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "provinces A";
			$sqlWhere = $where->sql;
