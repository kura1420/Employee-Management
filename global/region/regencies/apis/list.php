...
/**
 * global/region/regencies/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header regencies (regencies)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends regenciesBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\regencies_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new regencies_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'id' => 'A.`id`', 'name' => 'A.`name`', 'province_id' => 'A.`province_id`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "regencies A";
			$sqlWhere = $where->sql;
