...
/**
 * global/region/villages/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header villages (villages)
 ...
 * tanggal 13/06/2024
 */
$API = new class extends villagesBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\villages_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new villages_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'id' => 'A.`id`', 'name' => 'A.`name`', 'district_id' => 'A.`district_id`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "villages A";
			$sqlWhere = $where->sql;
