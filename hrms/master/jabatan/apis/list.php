...
/**
 * hrms/master/jabatan/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header jabatan (mst_jabatan)
 ...
 * tanggal 05/06/2024
 */
$API = new class extends jabatanBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\jabatan_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new jabatan_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.jabatan_id LIKE CONCAT('%', :search, '%') OR A.jabatan_nama LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'jabatan_id' => 'A.`jabatan_id`', 'jabatan_nama' => 'A.`jabatan_nama`', 'jabatan_isdisabled' => 'A.`jabatan_isdisabled`', '_createby' => 'A.`_createby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_jabatan A";
			$sqlWhere = $where->sql;
