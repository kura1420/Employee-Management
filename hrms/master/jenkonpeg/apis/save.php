...
/**
 * hrms/master/jenkonpeg/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header jenkonpeg (mst_jenkonpeg)
 *
 ...
 * tanggal 05/06/2024
 */
$API = new class extends jenkonpegBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_jenkonpeg';
		$primarykey = 'jenkonpeg_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\jenkonpeg_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new jenkonpeg_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->jenkonpeg_id = strtoupper($obj->jenkonpeg_id);


			if ($obj->jenkonpeg_descr=='') { $obj->jenkonpeg_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"jenkonpeg_id" => $obj->jenkonpeg_id
				];

				$criteriaValues = [
					"jenkonpeg_id" => " jenkonpeg_id = :jenkonpeg_id "
				];
				...
				$sqlFieldList = [
					'jenkonpeg_id' => 'A.`jenkonpeg_id`', 'jenkonpeg_name' => 'A.`jenkonpeg_name`', 'jenkonpeg_descr' => 'A.`jenkonpeg_descr`', 'jenkonpeg_isdisabled' => 'A.`jenkonpeg_isdisabled`',
					'jenkonpeg_ispermanent' => 'A.`jenkonpeg_ispermanent`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_jenkonpeg A";
				$sqlWhere = $where->sql;
