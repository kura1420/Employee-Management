...
/**
 * hrms/master/pegawai/apis/pegawaialmt-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawaialmtHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawaialmt_id', 'province_id', 'regency_id', 'district_id',
					'villages_id', 'pegawaialmt_kodepos', 'pegawaialmt_alamat', 'pegawai_id'
			];
			...
			if ($obj->pegawaialmt_kodepos=='') { $obj->pegawaialmt_kodepos = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawaialmt_id" => $obj->pegawaialmt_id
				];
