...
/**
 * hrms/master/pegawai/apis/pegawaiattch-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawaiattchHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawaiattch_id', 'pegawaiattch_number', 'pegawaiattch_file', 'pegawaiattch_isexpired',
					'pegawaiattch_expired', 'pegawai_id'
			];
			...
			$obj->pegawaiattch_expired = (\DateTime::createFromFormat('d/m/Y',$obj->pegawaiattch_expired))->format('Y-m-d');



			if ($obj->pegawaiattch_file=='') { $obj->pegawaiattch_file = '--NULL--'; }
			if ($obj->pegawaiattch_expired=='') { $obj->pegawaiattch_expired = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawaiattch_id" => $obj->pegawaiattch_id
				];
				...
				$dataresponse = array_merge($record, [
					...
					'pegawaiattch_expired' => date("d/m/Y", strtotime($row['pegawaiattch_expired'])),
