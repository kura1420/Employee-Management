...
if (is_file(__DIR__ .'/data-pegawaicard-handler.php')) {
	require_once __DIR__ .'/data-pegawaicard-handler.php';
}
...
/**
 * hrms/master/pegawai/apis/pegawaicard-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel pegawaicard pegawai (mst_pegawaicard)
 *
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_pegawaicard';
		$primarykey = 'pegawaicard_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\pegawai_pegawaicardHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawaicardHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawaicard_id', 'card_id', 'pegawaicard_number', 'pegawaicard_isexpired',
					'pegawaicard_expired', 'pegawaicard_file', 'pegawai_id'
			];
			...
			$obj->pegawaicard_expired = (\DateTime::createFromFormat('d/m/Y',$obj->pegawaicard_expired))->format('Y-m-d');



			if ($obj->pegawaicard_expired=='') { $obj->pegawaicard_expired = '--NULL--'; }
			if ($obj->pegawaicard_file=='') { $obj->pegawaicard_file = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawaicard_id" => $obj->pegawaicard_id
				];

				$criteriaValues = [
					"pegawaicard_id" => " pegawaicard_id = :pegawaicard_id "
				];
				...
				$sqlFieldList = [
					'pegawaicard_id' => 'A.`pegawaicard_id`', 'card_id' => 'A.`card_id`', 'pegawaicard_number' => 'A.`pegawaicard_number`', 'pegawaicard_isexpired' => 'A.`pegawaicard_isexpired`',
					'pegawaicard_expired' => 'A.`pegawaicard_expired`', 'pegawaicard_file' => 'A.`pegawaicard_file`', 'pegawai_id' => 'A.`pegawai_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_pegawaicard A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'pegawaicard_expired' => date("d/m/Y", strtotime($row['pegawaicard_expired'])),
