...
/**
 * hrms/master/pegawai/apis/pegawaikerja-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawaikerjaHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawaikerja_id', 'pegawaikerja_nama', 'pegawaikerja_thnmasuk', 'pegawaikerja_thnkeluar',
					'pegawaikerja_jabatan', 'pegawaikerja_descr', 'pegawaikerja_alasan', 'pegawaikerja_file',
					'pegawai_id'
			];
			...
			if ($obj->pegawaikerja_file=='') { $obj->pegawaikerja_file = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawaikerja_id" => $obj->pegawaikerja_id
				];
