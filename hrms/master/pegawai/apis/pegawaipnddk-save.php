...
if (is_file(__DIR__ .'/data-pegawaipnddk-handler.php')) {
	require_once __DIR__ .'/data-pegawaipnddk-handler.php';
}
...
/**
 * hrms/master/pegawai/apis/pegawaipnddk-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel pegawaipnddk pegawai (mst_pegawaipnddk)
 *
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_pegawaipnddk';
		$primarykey = 'pegawaipnddk_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\pegawai_pegawaipnddkHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawaipnddkHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawaipnddk_id', 'pnddk_id', 'pegawaipnddk_nama', 'pegawaipnddk_jurusan',
					'pegawaipnddk_lulus', 'regency_id', 'pegawaipnddk_file', 'pegawai_id'
			];
			...
			if ($obj->pegawaipnddk_jurusan=='') { $obj->pegawaipnddk_jurusan = '--NULL--'; }
			if ($obj->pegawaipnddk_file=='') { $obj->pegawaipnddk_file = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawaipnddk_id" => $obj->pegawaipnddk_id
				];

				$criteriaValues = [
					"pegawaipnddk_id" => " pegawaipnddk_id = :pegawaipnddk_id "
				];
				...
				$sqlFieldList = [
					'pegawaipnddk_id' => 'A.`pegawaipnddk_id`', 'pnddk_id' => 'A.`pnddk_id`', 'pegawaipnddk_nama' => 'A.`pegawaipnddk_nama`', 'pegawaipnddk_jurusan' => 'A.`pegawaipnddk_jurusan`',
					'pegawaipnddk_lulus' => 'A.`pegawaipnddk_lulus`', 'regency_id' => 'A.`regency_id`', 'pegawaipnddk_file' => 'A.`pegawaipnddk_file`', 'pegawai_id' => 'A.`pegawai_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_pegawaipnddk A";
				$sqlWhere = $where->sql;
