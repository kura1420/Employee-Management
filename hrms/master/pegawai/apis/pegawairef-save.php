...
if (is_file(__DIR__ .'/data-pegawairef-handler.php')) {
	require_once __DIR__ .'/data-pegawairef-handler.php';
}
...
/**
 * hrms/master/pegawai/apis/pegawairef-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel pegawairef pegawai (mst_pegawairef)
 *
...
$API = new class extends pegawaiBase {
	...
	public function execute($data, $options, $files) {
		$event = 'on-save';
		$tablename = 'mst_pegawairef';
		$primarykey = 'pegawairef_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\pegawai_pegawairefHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new pegawai_pegawairefHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'pegawairef_id', 'pegawairef_nama', 'pegawairef_hubungan', 'pegawairef_handphone',
					'pegawai_id'
			];
			...
			if ($obj->pegawairef_handphone=='') { $obj->pegawairef_handphone = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"pegawairef_id" => $obj->pegawairef_id
				];

				$criteriaValues = [
					"pegawairef_id" => " pegawairef_id = :pegawairef_id "
				];
				...
				$sqlFieldList = [
					'pegawairef_id' => 'A.`pegawairef_id`', 'pegawairef_nama' => 'A.`pegawairef_nama`', 'pegawairef_hubungan' => 'A.`pegawairef_hubungan`', 'pegawairef_handphone' => 'A.`pegawairef_handphone`',
					'pegawai_id' => 'A.`pegawai_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_pegawairef A";
				$sqlWhere = $where->sql;
