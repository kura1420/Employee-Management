...
import * as hnd from  './pegawai-pegawaialmtgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaialmtgrid-tbl_list');
const txt_title = $('#pnl_editpegawaialmtgrid-title');
const pnl_control = $('.pnl_editpegawaialmtgrid-control');
const btn_removechecked  = $('#pnl_editpegawaialmtgrid-removechecked');
const btn_addrow = $('#pnl_editpegawaialmtgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaialmt_id)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaialmt-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaialmtform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaialmtgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaialmtgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaialmtform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaialmtform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaialmt-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaialmtform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaialmtform'].handler.createnew(header_data)
	})	
}