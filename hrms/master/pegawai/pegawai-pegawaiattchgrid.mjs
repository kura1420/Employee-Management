...
import * as hnd from  './pegawai-pegawaiattchgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaiattchgrid-tbl_list');
const txt_title = $('#pnl_editpegawaiattchgrid-title');
const pnl_control = $('.pnl_editpegawaiattchgrid-control');
const btn_removechecked  = $('#pnl_editpegawaiattchgrid-removechecked');
const btn_addrow = $('#pnl_editpegawaiattchgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaiattch_number)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaiattch-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaiattchform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaiattchgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaiattchgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaiattchform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaiattchform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaiattch-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaiattchform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaiattchform'].handler.createnew(header_data)
	})	
}