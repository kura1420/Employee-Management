...
import * as hnd from  './pegawai-pegawaicardgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaicardgrid-tbl_list');
const txt_title = $('#pnl_editpegawaicardgrid-title');
const pnl_control = $('.pnl_editpegawaicardgrid-control');
const btn_removechecked  = $('#pnl_editpegawaicardgrid-removechecked');
const btn_addrow = $('#pnl_editpegawaicardgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaicard_number)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaicard-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaicardform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaicardgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaicardgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaicardform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaicardform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaicard-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaicardform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaicardform'].handler.createnew(header_data)
	})	
}