...
import * as hnd from  './pegawai-pegawaikelgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaikelgrid-tbl_list');
const txt_title = $('#pnl_editpegawaikelgrid-title');
const pnl_control = $('.pnl_editpegawaikelgrid-control');
const btn_removechecked  = $('#pnl_editpegawaikelgrid-removechecked');
const btn_addrow = $('#pnl_editpegawaikelgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaikel-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaikelform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaikelgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaikelgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaikelform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaikelform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaikel-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaikelform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaikelform'].handler.createnew(header_data)
	})	
}