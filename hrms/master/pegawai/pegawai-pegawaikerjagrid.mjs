...
import * as hnd from  './pegawai-pegawaikerjagrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaikerjagrid-tbl_list');
const txt_title = $('#pnl_editpegawaikerjagrid-title');
const pnl_control = $('.pnl_editpegawaikerjagrid-control');
const btn_removechecked  = $('#pnl_editpegawaikerjagrid-removechecked');
const btn_addrow = $('#pnl_editpegawaikerjagrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaikerja_nama)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaikerja-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaikerjaform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaikerjagrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaikerjagrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaikerjaform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaikerjaform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaikerja-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaikerjaform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaikerjaform'].handler.createnew(header_data)
	})	
}