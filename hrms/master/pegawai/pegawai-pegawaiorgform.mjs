...
import * as hnd from  './pegawai-pegawaiorgform-hnd.mjs'

const reload_header_modified = true;

const txt_caption = $('#pnl_editpegawaiorgform-caption')
const txt_title = $('#pnl_editpegawaiorgform-title')
const btn_edit = $('#pnl_editpegawaiorgform-btn_edit')
const btn_save = $('#pnl_editpegawaiorgform-btn_save')
const btn_delete = $('#pnl_editpegawaiorgform-btn_delete')
const btn_prev = $('#pnl_editpegawaiorgform-btn_prev')
const btn_next = $('#pnl_editpegawaiorgform-btn_next')
const btn_addnew = $('#pnl_editpegawaiorgform-btn_addnew')
const chk_autoadd = $('#pnl_editpegawaiorgform-autoadd')


const pnl_form = $('#pnl_editpegawaiorgform-form')
const obj = {
	txt_pegawaiorg_id: $('#pnl_editpegawaiorgform-txt_pegawaiorg_id'),
	txt_pegawaiorg_nama: $('#pnl_editpegawaiorgform-txt_pegawaiorg_nama'),
	txt_pegawaiorg_sebagai: $('#pnl_editpegawaiorgform-txt_pegawaiorg_sebagai'),
	cbo_regency_id: $('#pnl_editpegawaiorgform-cbo_regency_id'),
	txt_pegawaiorg_tahun: $('#pnl_editpegawaiorgform-txt_pegawaiorg_tahun'),
	txt_pegawaiorg_descr: $('#pnl_editpegawaiorgform-txt_pegawaiorg_descr'),
	txt_pegawai_id: $('#pnl_editpegawaiorgform-txt_pegawai_id')
}
...
export async function init(opt) {
	...
	form = new global.fgta4form(pnl_form, {
		primary: obj.txt_pegawaiorg_id,
		autoid: true,
		logview: 'mst_pegawaiorg',
		btn_edit: btn_edit,
		btn_save: btn_save,
	...
	obj.cbo_regency_id.name = 'pnl_editpegawaiorgform-cbo_regency_id'		
	...
	document.addEventListener('OnButtonBack', (ev) => {
		...
		if ($ui.getPages().getCurrentPage()==this_page_id) {
			...
			if (form.isDataChanged()) {
				form.canceledit(()=>{
					$ui.getPages().show('pnl_editpegawaiorggrid', ()=>{
						form.setViewMode()
						$ui.getPages().ITEMS['pnl_editpegawaiorggrid'].handler.scrolllast()
					})					
				})
			} else {
				$ui.getPages().show('pnl_editpegawaiorggrid', ()=>{
...
export function open(data, rowid, hdata) {
	...
	txt_title.html(header_data.pegawaiorg_nama)
	if (typeof hnd!=='undefined') { 
	...
	var fn_dataopening = async (options) => {
		options.api = `${global.modulefullname}/pegawaiorg-open`
...
export function createnew(hdata) {
	...
	form.createnew(async (data, options)=>{
		...
		data.pegawaiorg_value = 0

		data.pegawaiorg_tahun = 0
...
async function form_datasaving(data, options) {
	options.api = `${global.modulefullname}/pegawaiorg-save`
...
async function form_datasaved(result, options) {
	...
	*/
	...
	form.rowid = $ui.getPages().ITEMS['pnl_editpegawaiorggrid'].handler.updategrid(data, form.rowid)
	var rowdata = {
		data: data,
		rowid: form.rowid
	}
...
async function form_deleting(data, options) {
	options.api = `${global.modulefullname}/pegawaiorg-delete`
...
async function form_deleted(result, options) {
	...
	$ui.getPages().show('pnl_editpegawaiorggrid', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaiorggrid'].handler.removerow(form.rowid)
	});
...
function form_idsetup(options) {
	var objid = obj.txt_pegawaiorg_id
...
function btn_prev_click() {
	...
	var record = $ui.getPages().ITEMS['pnl_editpegawaiorggrid'].handler.getGrid().DATA[dataid]
