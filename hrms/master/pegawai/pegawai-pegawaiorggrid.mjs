...
import * as hnd from  './pegawai-pegawaiorggrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaiorggrid-tbl_list');
const txt_title = $('#pnl_editpegawaiorggrid-title');
const pnl_control = $('.pnl_editpegawaiorggrid-control');
const btn_removechecked  = $('#pnl_editpegawaiorggrid-removechecked');
const btn_addrow = $('#pnl_editpegawaiorggrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaiorg_nama)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaiorg-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaiorgform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaiorggrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaiorggrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaiorgform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaiorgform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaiorg-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaiorgform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaiorgform'].handler.createnew(header_data)
	})	
}