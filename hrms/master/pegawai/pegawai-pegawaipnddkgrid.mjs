...
import * as hnd from  './pegawai-pegawaipnddkgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaipnddkgrid-tbl_list');
const txt_title = $('#pnl_editpegawaipnddkgrid-title');
const pnl_control = $('.pnl_editpegawaipnddkgrid-control');
const btn_removechecked  = $('#pnl_editpegawaipnddkgrid-removechecked');
const btn_addrow = $('#pnl_editpegawaipnddkgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaipnddk-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaipnddkform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaipnddkgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaipnddkgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaipnddkform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaipnddkform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaipnddk-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaipnddkform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaipnddkform'].handler.createnew(header_data)
	})	
}