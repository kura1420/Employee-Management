...
import * as hnd from  './pegawai-pegawaipnddknogrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawaipnddknogrid-tbl_list');
const txt_title = $('#pnl_editpegawaipnddknogrid-title');
const pnl_control = $('.pnl_editpegawaipnddknogrid-control');
const btn_removechecked  = $('#pnl_editpegawaipnddknogrid-removechecked');
const btn_addrow = $('#pnl_editpegawaipnddknogrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawaipnddkno_nama)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawaipnddkno-list`
		options.criteria['id'] = data.pegawai_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawaipnddknoform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawaipnddknogrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawaipnddknogrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawaipnddknoform', () => {
		$ui.getPages().ITEMS['pnl_editpegawaipnddknoform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawaipnddkno-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawaipnddknoform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawaipnddknoform'].handler.createnew(header_data)
	})	
}