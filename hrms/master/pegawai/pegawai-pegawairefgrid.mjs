...
import * as hnd from  './pegawai-pegawairefgrid-hnd.mjs'

const tbl_list = $('#pnl_editpegawairefgrid-tbl_list');
const txt_title = $('#pnl_editpegawairefgrid-title');
const pnl_control = $('.pnl_editpegawairefgrid-control');
const btn_removechecked  = $('#pnl_editpegawairefgrid-removechecked');
const btn_addrow = $('#pnl_editpegawairefgrid-addrow');
...
export function createnew(data, options) {
	...
	txt_title.html(data.pegawairef_nama)
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/pegawairef-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editpegawairefform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editpegawairefgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editpegawairefgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editpegawairefform', () => {
		$ui.getPages().ITEMS['pnl_editpegawairefform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/pegawairef-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editpegawairefform', ()=>{
		$ui.getPages().ITEMS['pnl_editpegawairefform'].handler.createnew(header_data)
	})	
}