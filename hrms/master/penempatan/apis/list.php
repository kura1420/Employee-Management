...
/**
 * hrms/master/penempatan/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header penempatan (mst_penempatan)
 ...
 * tanggal 09/06/2024
 */
$API = new class extends penempatanBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\penempatan_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new penempatan_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.penempatan_id LIKE CONCAT('%', :search, '%') OR A.penempatan_nama LIKE CONCAT('%', :search, '%') OR A.penempatan_prov LIKE CONCAT('%', :search, '%') OR A.penempatan_kota LIKE CONCAT('%', :search, '%') OR A.penempatan_telp LIKE CONCAT('%', :search, '%') OR A.penempatan_email LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'penempatan_id' => 'A.`penempatan_id`', 'penempatan_nama' => 'A.`penempatan_nama`', 'penempatan_telp' => 'A.`penempatan_telp`', 'penempatan_email' => 'A.`penempatan_email`',
				'province_id' => 'A.`province_id`', 'regency_id' => 'A.`regency_id`', 'penempatan_alamat' => 'A.`penempatan_alamat`', 'penempatan_isdisabled' => 'A.`penempatan_isdisabled`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_penempatan A";
			$sqlWhere = $where->sql;
