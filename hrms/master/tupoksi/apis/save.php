...
/**
 * hrms/master/tupoksi/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header tupoksi (mst_tupoksi)
 *
 ...
 * tanggal 09/06/2024
 */
$API = new class extends tupoksiBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_tupoksi';
		$primarykey = 'tupoksi_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\tupoksi_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new tupoksi_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"tupoksi_id" => $obj->tupoksi_id
				];

				$criteriaValues = [
					"tupoksi_id" => " tupoksi_id = :tupoksi_id "
				];
				...
				$sqlFieldList = [
					'tupoksi_id' => 'A.`tupoksi_id`', 'dept_id' => 'A.`dept_id`', 'divisi_id' => 'A.`divisi_id`', 'jabatan_id' => 'A.`jabatan_id`',
					'tupoksi_tugas' => 'A.`tupoksi_tugas`', 'tupoksi_fungsi' => 'A.`tupoksi_fungsi`', 'tupoksi_isdisabled' => 'A.`tupoksi_isdisabled`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_tupoksi A";
				$sqlWhere = $where->sql;
