...
/**
 * hrms/transaction/konpeg/apis/save.php
 *
 * ====
 * Save
 * ====
...
$API = new class extends konpegBase {
	...
	public function execute($data, $options, $files) {
		...
		try {
			...
			$obj->konpeg_dtmulai = (\DateTime::createFromFormat('d/m/Y',$obj->konpeg_dtmulai))->format('Y-m-d');
			$obj->konpeg_dtberakhir = (\DateTime::createFromFormat('d/m/Y',$obj->konpeg_dtberakhir))->format('Y-m-d');

			$obj->konpeg_id = strtoupper($obj->konpeg_id);
			$obj->konpeg_file = strtoupper($obj->konpeg_file);


			if ($obj->konpeg_periodval=='') { $obj->konpeg_periodval = '--NULL--'; }
			if ($obj->konpeg_dtberakhir=='') { $obj->konpeg_dtberakhir = '--NULL--'; }
			if ($obj->konpeg_file=='') { $obj->konpeg_file = '--NULL--'; }
			if ($obj->konpeg_catatan=='') { $obj->konpeg_catatan = '--NULL--'; }


			unset($obj->konpeg_iswork);
			...
			try {
				...
				$dataresponse = array_merge($record, [
					...
					'konpeg_dtmulai' => date("d/m/Y", strtotime($row['konpeg_dtmulai'])),
					'konpeg_dtberakhir' => date("d/m/Y", strtotime($row['konpeg_dtberakhir'])),
